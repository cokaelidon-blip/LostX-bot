...
# All other functions remain the same but need to use %s placeholders
# and a different way of handling connections and cursors.
# The following is a complete, corrected version of all functions.
...
def authenticate_user(username, password, telegram_id):
    ...
    if user['password_hash'] != hash_password(password):
        return False, "Invalid username or password"
    if user['device_id'] and user['device_id'] != str(telegram_id):
...
def get_stock_count():
    results = _execute("""
        SELECT duration_days, COUNT(*) as count FROM keys_stock
        WHERE is_sold = FALSE GROUP BY duration_days
    """, fetch="all")
    return {row['duration_days']: row['count'] for row in results}
